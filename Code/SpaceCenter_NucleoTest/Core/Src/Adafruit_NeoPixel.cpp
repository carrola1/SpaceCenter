...
#include "../Inc/Adafruit_NeoPixel.hpp"
#include <stdlib.h>
...
*/
Adafruit_NeoPixel::Adafruit_NeoPixel(uint16_t n, GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin, neoPixelType t) :
  begun(false), brightness(0), pixels(NULL), endTime(0) {
  updateType(t);
  updateLength(n);
  setPin(GPIOx, GPIO_Pin);
}
...
Adafruit_NeoPixel::Adafruit_NeoPixel() :
  begun(false), numLEDs(0), numBytes(0), brightness(0), pixels(NULL),
...
void Adafruit_NeoPixel::show(void) {
  ...
  p       = *ptr++;
  bitMask =  0x80;
  uint32_t gpioMask = gpioPort->ODR; // Get current state of output register
  uint32_t gpioSet = 1 << (gpioPin-1);
  uint32_t gpioClear = 0;
  ...
  if(is800KHz) {
    ...
    for(;;) {
      if(p & bitMask) { // ONE
        // High 800ns (original code - 94 nop)
    	gpioPort->ODR = (gpioSet | gpioMask);
        //asm("nop;");
        // Low 450ns (original code - 38 nop)
        gpioPort->ODR = (gpioClear | gpioMask);
        //asm("nop;");
      } else { // ZERO
        // High 400ns (original code - 41 nop)
    	gpioPort->ODR = (gpioSet | gpioMask);
        //asm("nop;");
        // Low 850ns (original code - 92 nop)
        gpioPort->ODR = (gpioClear | gpioMask);
        //asm("nop;");
      }
      if(bitMask >>= 1) {
...
*/
void Adafruit_NeoPixel::setPin(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin) {
  gpioPort = GPIOx;
  gpioPin = GPIO_Pin;
}
